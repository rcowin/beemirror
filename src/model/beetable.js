...
const tableHead = {
  attrs: {columns: {default: 1}},
  parseDOM: [{tag: "thead", getAttrs(dom) {
    let row = dom.querySelector("tr")
    if (!row || !row.children.length) return false
    // FIXME using the child count as column width is problematic
    // when parsing document fragments
...
const tableBody = {
  attrs: {columns: {default: 1}},
  parseDOM: [{tag: "tbody", getAttrs(dom) {
    let row = dom.querySelector("tr")
    if (!row || !row.children.length) return false
    // FIXME using the child count as column width is problematic
    // when parsing document fragments
...
class AddColumnStep extends Step {
  ...
  apply(doc) {
    let index = null, table = null, tablePos = null, rowContainer = null
    for (let i = 0; i < this.positions.length; i++) {
      ...
      if (table == null) {
        rowContainer = $pos.node(-1)
        table = $pos.node(-2)

        if (rowContainer.childCount != this.positions.length)
          return StepResult.fail("Mismatch in number of rows")
        tablePos = $pos.before(-2)
        index = $pos.index()
      } else if ($pos.before(-2) != tablePos || $pos.index() != index) {
        return StepResult.fail("Column insert positions not consistent")
      }
    }

    let updatedContainers = []
    for (let k = 0; k < table.childCount; k++) {
      let rowContainer = table.child(k)
      let updatedRows = []

      for (let i = 0; i < rowContainer.childCount; i++) {
        let row = rowContainer.child(i), rowCells = index ? [] : [this.cells[i]]
        for (let j = 0; j < row.childCount; j++) {
          rowCells.push(row.child(j))
          if (j + 1 == index) rowCells.push(this.cells[0])
        }
        updatedRows.push(row.type.create(adjustColumns(row.attrs, 1), Fragment.from(rowCells)))
      }
      updatedContainers.push(rowContainer.type.create(adjustColumns(rowContainer.attrs, 1),  Fragment.from(updatedRows)))
    }
    let updatedTable = table.type.create(adjustColumns(table.attrs, 1),  Fragment.from(updatedContainers))
...
class RemoveColumnStep extends Step {
  ...
  apply(doc) {
    ...
    for (let i = 0; i < this.from.length; i++) {
      ...
      if (table == null) {
        rowContainer = $from.node(-1)
        table = $from.node(-2)
        if (rowContainer.childCount != this.from.length)
          return StepResult.fail("Mismatch in number of rows")
        tablePos = $from.before(-2)
        index = $from.index()
      } else if ($from.before(-2) != tablePos || $from.index() != index) {
    ...
    for (let k = 0; k < table.childCount; k++) {
      ...
      let updatedRows = []
      for (let i = 0; i < rowContainer.childCount; i++) {
        let row = rowContainer.child(i), rowCells = []
        for (let j = 0; j < row.childCount; j++)
